...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use hru_module, only : hru, sedyld, ihru, sedminpa, sedminps, sedorgp, sedorgn, surqsolp, surqno3,   &
         latno3, clayld, silyld, sanyld, sagyld, lagyld ! KDW 08/02
...
!! Sediment
	sedyld(j) = sedyld(j) * (1 - hru(j)%lumv%bmp_sed/100)
      clayld(j) = clayld(j) * (1 - hru(j)%lumv%bmp_sed/100) ! KDW 08/02
      silyld(j) = silyld(j) * (1 - hru(j)%lumv%bmp_sed/100) ! KDW 08/02
      sanyld(j) = sanyld(j) * (1 - hru(j)%lumv%bmp_sed/100) ! KDW 08/02
      sagyld(j) = sagyld(j) * (1 - hru(j)%lumv%bmp_sed/100) ! KDW 08/02
      lagyld(j) = lagyld(j) * (1 - hru(j)%lumv%bmp_sed/100) ! KDW 08/02
