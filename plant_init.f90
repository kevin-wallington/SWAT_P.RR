...
!      use hru_lte_module
      ...
      !!assign land use pointers for the hru 
        ! hru(j)%land_use_mgt = ilu ! lines 63-65 moved to hrudb_init by KDW, 12/21/21
        ...
        ! hru(j)%mgt_ops = lum_str(ilu)%mgt_ops  ! lines 76-83 moved to hrudb_init by KDW, 12/21/21
      ...
      !! allocate plants
        ...
        cvm_com(j) = -7.0 ! KDW 12/20/21, since cvm<0 and want to take max from plants in community, start at -7 (approximately usle_c_min = 0.001)
        blai_com(j) = 0.
        rsdco_plcom(j) = 0.
        pcom(j)%pcomdb = icom
        pcom(j)%rot_yr = 1
        do ipl = 1, pcom(j)%npl
          ...
          !cvm_com(j) = plcp(idp)%cvm + cvm_com(j)   ! replaced below by KDw
          cvm_com(j) = max(plcp(idp)%cvm, cvm_com(j)) ! KDW 12/20/21, takes largest mininum usle c (cvm = ln(c) and c< 1, so adding was causing cvm to become too small)
          ...
          blai_com(j) = blai_com(j) + pcom(j)%plcur(ipl)%lai_pot ! KDW 04/08
