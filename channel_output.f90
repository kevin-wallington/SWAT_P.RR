...
!!!!! monthly print
      if (time%end_mo == 1) then
        ...
        const = float (ndays(time%mo + 1) - ndays(time%mo)) !! KDW
        ch_m(jrch) = ch_m(jrch) / const            !! convert to daily average values - KDW
...
!!!!! yearly print
      if (time%end_yr == 1) then
        ...
        const = time%day_end_yr !! KDW
        ch_y(jrch) = ch_y(jrch) / const      !! converts to daily average values - KDW
...
100   format (4i6,2i8,2x,a,75e14.4) !75 was 60 - KDW, 06/04 14.4 was 15.4 
      return
