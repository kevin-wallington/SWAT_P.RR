...
!!    set default values for undefined parameters
      ...
      if (ch_nut(ich)%w12 <= 0.) ch_nut(ich)%w12 = .005
      if (ch_nut(ich)%cbod_half <= 0.) ch_nut(ich)%cbod_half = 40.
      if (ch_nut(ich)%dox_half <= 0.) ch_nut(ich)%dox_half = 3.
      if (ch_nut(ich)%srpm2 <= 0.) ch_nut(ich)%srpm2 = 0.002
      if (ch_nut(ich)%adskeq <= 0.) ch_nut(ich)%adskeq = 0.01 ! KDW 07/27
...
!!    set default values for undefined parameters
      ...
      if (ch_nut(ich)%rs2 <= -10.0) ch_nut(ich)%rs2 = 0.05 ! KDW 05/12/22, change limit to -1 so, under old model, can have reach be sink
