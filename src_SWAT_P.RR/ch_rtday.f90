...
! store floodplain water that can be used by riparian HRU"s        
      ...
      if (sdti > 0.) then
       ...
       !! calculate transmission losses
	    ...
	    if (rtwtr > 0.1) then ! changed from > 0. to > 0.1, KDW
      ...
      if (rtwtr < 1.e-3) rtwtr = 0. ! changed from < 0. to < 1.e-3, KDW
      if (ch(jrch)%rchstor < 1.e-3) ch(jrch)%rchstor = 0. ! changed from < 0. to < 1.e-3, KDW
