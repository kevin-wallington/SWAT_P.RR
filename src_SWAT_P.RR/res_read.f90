      ...
      use channel_module, only: streambed_init_name ! KDW 11/04/21
       ...
       do i = 1, db_mx%res_dat
        ...
        do isp_ini = 1, db_mx%res_init
          if (res_dat_c(ires)%init == res_init_dat_c(isp_ini)%init) then
            ...
            !!! Below added by KDW 11/03/21
            do ics = 1, db_mx%streambed_init
              if (res_init_dat_c(isp_ini)%bed == streambed_init_name(ics)) then
                res_init(isp_ini)%bed = ics
                exit
              end if
            if (res_init(isp_ini)%bed == 0) write (9001,*) streambed_init_name(ics), " not found"
            end do
            !! End KDW addition
