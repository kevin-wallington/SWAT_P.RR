...
!!    hydrologic cycle
      ...
      use hru_module, only : hru, ihru, tmx, tmn, tmpav, hru_ra, hru_rmx, rhd, u10, tillage_switch,      &
         tillage_days, ndeat, qdr, phubase, sedyld, surfq, grz_days, yield, surfq_pervious, &
         ...
         precipday, precip_eff, qday, latqrunon, rls_solp, rls_no3, &
         sanyld, silyld, clayld, sagyld, lagyld
      use soil_module 
      use plant_module
      use basin_module
      ...
      weat = wst(iwst)%weat
      !precip_eff = precipday ! KDW, moved down 10/08
      !rls_solp = 0 ! - KDW, commented out 04/07
      !rls_no3 = 0 ! - KDW, commented out 04/07
      
      precipday = wst(iwst)%weat%precip
      precip_eff = precipday ! KDW, moved from above 10/08
      ...
      if (nd_30 > 30) nd_30 = 1
        ...
        if (ob(icmd)%hin_sur%flo > 1.e-6) then
          ...
          if (ires > 0) then
            ...
            wet(j) = wet(j) + ob(icmd)%hin_sur !Note: this adds runon immediately to wetland "tank", bypassing initial trapping; consider changing to ht1 = hin_sur - KDW
          else
        ...
        sanyld(j) = sanyld(j) + ht2%san ! - KDW
        silyld(j) = silyld(j) + ht2%sil ! - KDW
        clayld(j) = clayld(j) + ht2%cla ! - KDW
        sagyld(j) = sagyld(j) + ht2%sag ! - KDW
        lagyld(j) = lagyld(j) + ht2%lag ! - KDW


        !! compute effective rainfall (amount that percs into soil)
        !! add infiltration from surface runon
        if (hru(j)%luse%urb_lu > 0) then !If statement added by KDW 01/10/22
          inflpcp = (precip_eff - surfq_pervious(j)) * (1.-hru(j)%water_fr) + hru(j)%water_seep /(10.* hru(j)%area_ha) !If statement added by KDW 01/10/22
        else
        ...
        !! prior location of wetland_control, moved after "swr_substor" - KDW
        ...
        if (bsn_cc%sol_P_model == 0) then
            call nut_pminrl
        elseif (bsn_cc%sol_P_model == 1) then
            call nut_pminrl2
        else
            call nut_pminrl3 !! SWAT+P, simplified Langmuir isotherm - KDW
        end if
        ...
        !! ht2%orgp==organic P (kg) routed across hru from surface runon
        sedorgn(j) = sedorgn(j) + ht2%orgn / hru(j)%area_ha ! - KDW
        sedorgp(j) = sedorgp(j) + ht2%orgp / hru(j)%area_ha ! - KDW
        sedminpa(j) = sedminpa(j) + ht2%minpa / hru(j)%area_ha ! - KDW
        sedminps(j) = sedminps(j) + ht2%minps / hru(j)%area_ha ! - KDW
        ...
        !! wetland processes - moved down from prior location (after swr_percmain) by KDW
        hru(j)%water_fr = 0.
        if (ires > 0) then
	 ...
	 !! compute reduction in pollutants due to in field grass waterway
	   ...
	   if (hru(j)%lumv%bmp_flag == 1) then
        ...
        if (hru(j)%luse%urb_lu > 0) then ! if statement and code therein (if = true) added by KDW 01/10/22
            qdr(j) = qday + (1. - urbdb(ulu)%fimp) * latq(j) + (1. - urbdb(ulu)%fimp) * qtile
        else
            qdr(j) = qday + latq(j) + qtile
        endif
        
        !! ht2%flo is outflow from wetland or total saturation excess if no wetland (KDW: think it's actually always wetland outflow)
        ...
        !! compute chl-a, CBOD and dissolved oxygen loadings - moved from line 390 by KDW, 07/14
        call swr_subwq
      ...
      do iout = 1, ob(iob_out)%src_tot
          ...
          case ("cha")
            ...
            if (ob(iob_out)%htyp_out(iout) == "lat" .or. ob(iob_out)%htyp_out(iout) == "tot") then
              if (hru(j)%luse%urb_lu > 0) then ! if statement and code therein (if = true) added by KDW 01/10/22
                hwb_d(j)%latq_cha = hwb_d(j)%latq_cha + (1. - urbdb(ulu)%fimp) * latq(j) * ob(iob_out)%frac_out(iout)
              else
          ...
          case ("res")
            ...
            if (ob(iob_out)%htyp_out(iout) == "lat" .or. ob(iob_out)%htyp_out(iout) == "tot") then
              if (hru(j)%luse%urb_lu > 0) then ! if statement and code therein (if = true) added by KDW 01/10/22
                hwb_d(j)%latq_res = hwb_d(j)%latq_res + (1. - urbdb(ulu)%fimp) * latq(j) * ob(iob_out)%frac_out(iout)
              else
          ...
          case ("hru")
            ...
            if (ob(iob_out)%htyp_out(iout) == "lat" .or. ob(iob_out)%htyp_out(iout) == "tot") then
              if (hru(j)%luse%urb_lu > 0) then ! if statement and code therein (if = true) added by KDW 01/10/22
                hwb_d(j)%latq_ls = hwb_d(j)%latq_ls + (1. - urbdb(ulu)%fimp) * latq(j) * ob(iob_out)%frac_out(iout)
              else
      ...
      ! output_waterbal
        ...
        if (hru(j)%luse%urb_lu > 0) then ! if statement and code therein (if = true) added by KDW 01/10/22
          hwb_d(j)%latq = (1. - urbdb(ulu)%fimp) * latq(j)
          hwb_d(j)%perc = (1. - urbdb(ulu)%fimp) * sepbtm(j)
          hwb_d(j)%eplant = (1. - urbdb(ulu)%fimp) * ep_day
          hwb_d(j)%qtile = (1. - urbdb(ulu)%fimp) * qtile
        else
          hwb_d(j)%latq = latq(j)
      ...
      ! output_losses
        if (hru(j)%luse%urb_lu > 0) then ! if statement and code therein (if = true) added by KDW 01/10/22
            hls_d(j)%sedyld = (sedyld(j) + (1. - urbdb(ulu)%fimp) * latsed(j) + &
              (1. - urbdb(ulu)%fimp) * tilesed(j)) / hru(j)%area_ha
            hls_d(j)%sedorgn = sedorgn(j) + (1. - urbdb(ulu)%fimp) * latorgn(j) + (1. - urbdb(ulu)%fimp) * tileorgn(j)
            hls_d(j)%sedorgp = sedorgp(j)  + (1. - urbdb(ulu)%fimp) * latorgp(j) + (1. - urbdb(ulu)%fimp) * tileorgp(j)
            hls_d(j)%surqno3 = surqno3(j)
            hls_d(j)%latno3 = (1. - urbdb(ulu)%fimp) * latno3(j)
            hls_d(j)%surqsolp = surqsolp(j)
            hls_d(j)%usle = usle
            hls_d(j)%sedmin = sedminpa(j) + sedminps(j) + (1. - urbdb(ulu)%fimp) * latminpa(j) + & 
              (1. - urbdb(ulu)%fimp) * tileminpa(j) + (1. - urbdb(ulu)%fimp) * latminps(j) + & 
              (1. - urbdb(ulu)%fimp) * tileminps(j)
            hls_d(j)%tileno3 = (1. - urbdb(ulu)%fimp) * tileno3(j)
            hls_d(j)%latsolp = (1. - urbdb(ulu)%fimp) * latsolp(j)
            hls_d(j)%tilesolp = (1. - urbdb(ulu)%fimp) * tilesolp(j)
        else
            hls_d(j)%sedyld = (sedyld(j) + latsed(j) + tilesed(j)) / hru(j)%area_ha
            hls_d(j)%sedorgn = sedorgn(j) + latorgn(j) + tileorgn(j)
            hls_d(j)%sedorgp = sedorgp(j)  + latorgp(j) + tileorgp(j)
            hls_d(j)%surqno3 = surqno3(j)
            hls_d(j)%latno3 = latno3(j)
            hls_d(j)%surqsolp = surqsolp(j)
            hls_d(j)%usle = usle
            hls_d(j)%sedmin = sedminpa(j) + sedminps(j) + latminpa(j) + tileminpa(j) + &
                              latminps(j) + tileminps(j)
            hls_d(j)%tileno3 = tileno3(j)
            hls_d(j)%latsolp = latsolp(j)
            hls_d(j)%tilesolp = tilesolp(j)
        endif
