...
!!    initialize water/drainage coefs for each soil layer
      depth_prev = 0.
      sumpor = 0.
      soil(isol)%sumul = 0. ! KDW, 12/21/21
      soil(isol)%sumfc = 0. ! KDW, 12/21/21
      soil(isol)%sumwp = 0. ! KDW, 12/21/21
      do j = 1, nly
      ...
      if (sol(isol)%s%ffc > 1.) then
        !sol(isol)%s%wat_tbl = (sol(isol)%s%det_lag - sol(isol)%s%ffc *   & ! commented out and replaced with below by KDW 11/19/21
        ! sol(isol)%s%sumfc) / sol(isol)%phys(nly)%d
        sol(isol)%s%wat_tbl = sol(isol)%phys(nly)%d - (sol(isol)%s%ffc *   &
          sol(isol)%s%sumfc) / sol(isol)%phys(nly)%por
      else
