...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use hru_module, only : hru, ihru, cbodu, surfq, surqno3, surqsolp, sep_tsincefail, i_sep,  &
        isep, qday, sepday, surfq_pervious
      use soil_module
 	...
 	if (sep(isep)%opt == 2 .and. j1 == i_sep(j)) then
	    ...
	    ! Add surface ponding to the 10mm top layer when the top soil layer is saturated
	     ...
	     if (qlyr > 0) then
            ...
            if (hru(j)%luse%urb_lu > 0) then !if statement (and code therein, if = true) added by KDW 01/10/22
              ...
              surfq_pervious(j) = surfq_pervious(j) + qlyr
              surfq(j) = surfq(j) + (1. - urbdb(ulu)%fimp) * qlyr
            else
      ...
      if (j1 < soil(j)%nly) then
        if (soil(j)%phys(j1)%st - soil(j)%phys(j1)%ul > 1.e-4) then
          ...
          soil(j)%ly(j1)%prk = soil(j)%ly(j1)%prk + sepday ! KDW 11/29/21
        end if
      else
        ...
        if (soil(j)%phys(j1)%st - soil(j)%phys(j1)%ul > 1.e-4) then
          ...
          soil(j)%phys(j1)%st = soil(j)%phys(j1)%ul
          soil(j)%ly(j1-1)%prk = soil(j)%ly(j1-1)%prk - ul_excess ! KDW 11/29/21
          if (soil(j)%ly(j1-1)%prk < 1.e-6)  soil(j)%ly(j1-1)%prk = 0 ! KDW 11/29/21
          nn = soil(j)%nly
          do ly = nn - 1, 1, -1
            ...
            if (soil(j)%phys(ly)%st > soil(j)%phys(ly)%ul) then
              ...
              if (ly >= 2) then ! KDW 11/29/21
                soil(j)%ly(ly-1)%prk = soil(j)%ly(ly-1)%prk - ul_excess ! KDW 11/29/21
                if ( soil(j)%ly(ly-1)%prk < 1.e-6)  soil(j)%ly(ly-1)%prk = 0 ! KDW 11/29/21
              endif ! KDW 11/29/21
            else
              ul_excess = 0.
              exit
