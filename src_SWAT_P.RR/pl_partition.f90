      ...
      real :: biomxyr ! KDW from plant mortality, 12/15/21
      ...
      !!! Below is moved from pl_mortality by KDW, 12/15/21
      ...
      if (biomxyr > 1.e-6 .and. pl_mass(j)%tot(ipl)%m > biomxyr) then
        !rsd1(j)%tot(ipl)%m = pl_mass(j)%tot(ipl)%m - biomxyr ! Commented out by KDW. 2 reasons, (1) why should tree "growth" beyond limit go to residue instead of just capping growth?
        ! (2) this doesn't even add to the residue amount, just replaces it; should be rsd1(j)%tot(ipl)%m = rsd1(j)%tot(ipl)%m + ... if want to add to residue
        pl_mass(j)%tot(ipl)%m = biomxyr
      end if
      !!! Above is moved from pl_mortality by KDW
