...
!!    ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
	  ...
	  if (j1 == i_sep(j) .and. soil(j)%phys(j1)%tmp > 0. .and.          &
       ...
       !! determine gravity drained water in layer
        ...
        !qtile = qtile + lyrtile ! qtile not used before zeroed below, lyrtile is zero here, so just removed it, KDW 06/09
        soil(j)%ly(j1)%flat = latlyr !+ lyrtile ! lyrtile is zero here, so just removed it, KDW 06/09
      ...
      if (qtile > 0.) then
        ...
        sumqtile = qtile
        do j1 = 1, hru(j)%lumv%ldrain !soil(j)%nly changed to hru(j)%lumv%ldrain by KDW 11/29/21, no upward drainage
          xx = soil(j)%phys(j1)%st - soil(j)%phys(j1)%fc
          if (xx > 0.) then
            if (xx > sumqtile) then
              soil(j)%phys(j1)%st = soil(j)%phys(j1)%st - sumqtile
              !soil(j)%ly(j1)%prk = soil(j)%ly(j1)%prk + sumqtile ! KDW 11/29/21
              soil(j)%ly(j1)%qtilelyr = sumqtile ! KDW 06/09
              sumqtile = 0.
            else
              !soil(j)%ly(j1)%prk = soil(j)%ly(j1)%prk + xx ! KDW 11/29/21
              soil(j)%ly(j1)%qtilelyr = xx ! KDW 06/09
              sumqtile = sumqtile - xx
              soil(j)%phys(j1)%st = soil(j)%phys(j1)%fc
            end if
          else
              soil(j)%ly(j1)%qtilelyr = 0 ! KDW 11/29/21
          end if
        end do
        if (sumqtile > 0.) then
