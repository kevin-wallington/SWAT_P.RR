...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use hru_module, only : hru, ihru, ulu, twash, surfq, ubntss, surqno3, sedorgn, sedorgp, init_abstrc,  &
         surqsolp, ubnrunoff, isweep, phusw, phubase, etday, ipl , sedminpa, sedminps
      use plant_module
	  ...
	  do k = 1, time%step
          ...
          !! amounts are kg/ha
              ...
              ! replace old yield with those below, KDW 11/22/21
              sedorgn(j)= ((tn - tno3) / (hru(j)%km *100.))*urbdb(ulu)%fimp + sedorgn(j)
              surqno3(j)=(tno3  / (hru(j)%km *100.))*urbdb(ulu)%fimp + surqno3(j)
              ...
              ! end replacement by KDW 11/22/21
	      else
          !! no surface runoff
