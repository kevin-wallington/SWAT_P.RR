      ...
      do ichg_par = 1, db_mx%cal_upd
        do ispu = 1, cal_upd(ichg_par)%num_elem
          ...
          if (cond_met == "y") then
            ...
            case ("sol")
              ...
              !cal_lyr1 = cal_upd(ichg_par)%lyr1 ! commented out code that identified which layers to calibrate because incongruent with soil_awc_init, all soil layers calibrated together now, KDW 11/19/21
        ...
        case ("ovn") ! added by KDW to recalculate time of concentration if calibrate ovn; here because time_conc_init has own hru loop, 11/19/21
            call time_conc_init
        end select
      end do          ! ichg_par
