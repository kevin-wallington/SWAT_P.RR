       ...
       real :: uobp     !(not used in this model) none           |phosphorus uptake normalization parameter 
         ...
         if (bsn_prm%pperco < 1.e-6) bsn_prm%pperco = 1.0        !! phos perc coeff (0-1)
                                                                  !!  0 = conc of sol P in surf runoff is zero ! in *percolate* KDW, 11/30/21
                                                                  !!  1 = percolate has some conc of sol P as surf runoff ! same as *soil dissolved P conc*, KDW 11/30/21
         ...
         !! Added by KDW
         if (bsn_prm%srpm2 < 1.e-6) bsn_prm%srpm2 = 0.002          !! adsorption sites per kg (of clay) - KDW
         if (bsn_prm%adskeq < 1.e-6) bsn_prm%adskeq = 0.01         !! phosphorus adsorption equilibrium constant - KDW
         if (bsn_prm%sbdk < 1.e-6) bsn_prm%sbdk = 0.01         !! exponential rate of sediment embeddment in bed - KDW
