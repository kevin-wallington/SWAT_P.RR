...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !! compute residue decomp and mineralization of fresh organic n and p of flat residue
        do ipl = 1, pcom(j)%npl        !! we need to decompose each plant
          ...
          !if (rsd1(j)%tot(ipl)%n + rsd1(j)%mn%no3 > 1.e-4) then! revised to below since mineralized P routed to soil instead of litter layer, line 155 - KDW
          !if (rsd1(j)%tot(ipl)%n + soil1(j)%mn(1)%no3 > 1.e-4) then! - KDW
          if (rsd1(j)%tot(ipl)%n > 1.e-4) then! - KDW 12/15/21
            !cnr = r4 / (rsd1(j)%tot(ipl)%n  + rsd1(j)%mn%no3) ! revised to below since mineralized P routed to soil instead of litter layer, line 155 - KDW
            !cnr = r4 / (rsd1(j)%tot(ipl)%n  + soil1(j)%mn(1)%no3) ! - KDW
            cnr = r4 / rsd1(j)%tot(ipl)%n! - KDW 12/15/21
            if (cnr > 500.) cnr = 500.
          ...
          !if (rsd1(j)%tot(ipl)%p + rsd1(j)%mp%lab > 1.e-4) then ! revised to below since mineralized P routed to soil instead of litter layer, line 158 - KDW
          !if (rsd1(j)%tot(ipl)%p + soil1(j)%mp(1)%lab > 1.e-4) then ! - KDW
          if (rsd1(j)%tot(ipl)%p > 1.e-4) then ! - KDW 12/15/21
            !cpr = r4 / (rsd1(j)%tot(ipl)%p + rsd1(j)%mp%lab) ! revised to below since mineralized P routed to soil instead of litter layer, line 158 - KDW
            !cpr = r4 / (rsd1(j)%tot(ipl)%p + soil1(j)%mp(1)%lab) ! - KDW
            cpr = r4 / rsd1(j)%tot(ipl)%p ! - KDW 12/15/21
          ...
          soil1(j)%mn(1)%no3 = soil1(j)%mn(1)%no3 + .8 * rmn1 ! - KDW
          soil1(j)%hs(1)%n = soil1(j)%hs(1)%n + .2 * rmn1
          !rsd1(j)%mp%lab = rsd1(j)%mp%lab + .8 * rmp
          soil1(j)%mp(1)%lab = soil1(j)%mp(1)%lab + .8 * rmp ! - KDW
      ...
      do k = 1, soil(j)%nly
        ...
        if (soil(j)%phys(kk)%tmp > 0.) then
          ...
          rmp = 0.
          if (k <= 2) then
            !!! KDW - replaced all "tot" with "microb" below, b/c should be fresh organic pool, 10/15/21 
            r4 = .58 * soil1(j)%microb(k)%m
            if (soil1(j)%microb(k)%n + soil1(j)%mn(k)%no3 > 1.e-4) then
              cnr = r4 / (soil1(j)%microb(k)%n  + soil1(j)%mn(k)%no3)
              if (cnr > 500.) cnr = 500.
              cnrf = Exp(-.693 * (cnr - 25.) / 25.)
            else
              cnrf = 1.
            ...
            if (soil1(j)%microb(k)%p + soil1(j)%mp(k)%lab > 1.e-4) then
              cpr = r4 / (soil1(j)%microb(k)%p + soil1(j)%mp(k)%lab)
              if (cpr > 5000.) cpr = 5000.
              cprf = Exp(-.693 * (cpr - 200.) / 200.)
            else
              cprf = 1.
            ...
            decr = Max(bsn_prm%decr_min, decr)
            decr = Min(decr, 1.)
            soil1(j)%microb(k)%m = Max(1.e-6, soil1(j)%microb(k)%m)
            rdc = decr * soil1(j)%microb(k)%m
            soil1(j)%microb(k)%m = soil1(j)%microb(k)%m - rdc
            if (soil1(j)%microb(k)%m < 0.) soil1(j)%microb(k)%m = 0.
            !rmn1 = decr * (soil1(j)%str(k)%n + soil1(j)%lig(k)%n + soil1(j)%meta(k)%n) ! replaced with below: lignin, structural, and metabolic are part of cbn_zhang... - KDW
            rmn1 = decr * soil1(j)%microb(k)%n ! - KDW 10/15/21
            soil1(j)%microb(k)%p = Max(1.e-6, soil1(j)%microb(k)%p)
            !rmp = decr * (soil1(j)%str(k)%p + soil1(j)%lig(k)%p + soil1(j)%meta(k)%p) ! replaced with below: lignin, structural, and metabolic are part of cbn_zhang... - KDW
            rmp = decr * soil1(j)%microb(k)%p ! - KDW 10/15/21

            soil1(j)%microb(k)%p = soil1(j)%microb(k)%p - rmp
            soil1(j)%microb(k)%n  = Max(1.e-6, soil1(j)%microb(k)%n)
            soil1(j)%microb(k)%n  = soil1(j)%microb(k)%n  - rmn1
