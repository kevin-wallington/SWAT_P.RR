      ...
      type hydrology
           ...
           real :: tile_sed = 0.     !! lat_sed(:)    |g/L           |sediment concentration in lateral flow ! KDW, 06/23 line added
      ...
      type hydrologic_response_unit
        ...
        integer :: bmpuser = 0
        integer :: crop_reg = 0
        !real :: km_tot ! for urban HRUs, added to keep total area, b/c using area_ha and km for pervious area only, KDW 12/20/21
      ...
      real :: precip_eff_imp    !! mm   |daily precip for impervious area ! KDW 01/10/22
      real :: qday              !! mm   |surface runoff that reaches main channel during day in HRU 
      real :: rls_no3           !! kg   |NO3 in runon- KDW
      real :: rls_solp          !! kg   |Dissolved P in runon- KDW                       
...
!     New soluble P leaching variables - KDW
      real, dimension (:), allocatable :: latsolp, tilesolp
!     New variables to separately lag lateral and tile sediment and associated N/P - KDW
      real, dimension (:), allocatable :: latorgp, tileorgp
      real, dimension (:), allocatable :: latminpa, tileminpa
      real, dimension (:), allocatable :: latminps, tileminps
      real, dimension (:), allocatable :: latorgn, tileorgn   
      real, dimension (:), allocatable :: latsan,latsil,latcla,latsag
      real, dimension (:), allocatable :: latlag,latsed,tilesed
      real, dimension (:), allocatable :: tilesan,tilesil,tilecla
      real, dimension (:), allocatable :: tilesag, tilelag
      ...
      real, dimension (:), allocatable :: surfq_pervious ! KDW, for urban HRUs, 11/22/21
