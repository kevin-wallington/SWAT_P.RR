...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use basin_module
      use hru_module, only : hru, surfq, qtile, ihru, surfq_pervious
      use soil_module
      ...
      if (cs_db%num_pests /= 0) then
        ...
        do ly = 1, soil(j)%nly
          ...
          do k = 1, cs_db%num_pests
            ...
            if (ly == 1) then
              if (hru(j)%luse%urb_lu > 0) then ! KDW 01/10/22, if statement and code therein added
                vf = vf + surfq_pervious(j)
              else
                vf = vf + surfq(j)
              endif
            endif
