...
!!    Plus particle size tracking.
      ...
      ! aer_mass = .001 * 1 * (ch_vel(jrch)%wid_btm * 1000 * ch_hyd(jhyd)%l * ch_sed(jsed)%bed_bd) ! depth of aer layer is 1 ! moved outside qdin if statement, KDW 01/31/22
      ! anaer_mass = .1 * (ch_vel(jrch)%wid_btm * 1000 * ch_hyd(jhyd)%l * ch_sed(jsed)%bed_bd)  ! moved outside qdin if statement, KDW 01/31/22
      aer_mass = .002 * 1 * (ch_vel(jrch)%wid_btm * 1000 * ch_hyd(jhyd)%l * ch_sed(jsed)%bed_bd) ! depth of aer layer is 1 ! moved outside qdin if statement, KDW 01/31/22! KDW debug 04/28/22, test thicker layers, replacing above
      anaer_mass = .2 * (ch_vel(jrch)%wid_btm * 1000 * ch_hyd(jhyd)%l * ch_sed(jsed)%bed_bd)  ! moved outside qdin if statement, KDW 01/31/22! KDW debug 04/28/22, test thicker layers, replacing above

  !! initialize water in reach during time step
        qdin = 0.
        qdin = rtwtr_d + ch(jrch)%rchstor ! changed rtwtr to rtwtr_d, KDW 04/04/22
  ...
  !! initialize sediment in reach during time step
        ...
        fines_wtclm = clain + silin + sagin + sanin + 0.0001 ! moved up to preceed deposition and if qdin>0 loop, KDW 02/17/22
        sfarea_wtclm = clain + silin * .2 + sagin * (1./15.) + sanin * .01 + 0.0001 ! moved up to preceed deposition and if qdin>0 loop, KDW 02/17/22
  ...
  !! do not perform sediment routing if no water in reach
        if (rtwtr_d > 0. .and. rchdep > 0. .and. qdin > 0.01) then ! NEW - changed from below, KDW ! changed rtwtr to rtwtr_d, KDW 04/04/22
        !if (qdin > 0.01) then   
  ...
  !!    Potential Bed degradation rate in metric tons per day
        xyz = ch_sed(jsed)%bed_kd ! KDW debug 06/27/22
  ...
  !!    Relative potential for bed erosion
        ...
        if (ch_sed(jsed)%kod_a <= 0) then ! KDW 06/28
          !!    silt-bed rivers
            ...
            akod_a = ch_sed(jsed)%kod_a
            akod_b = ch_sed(jsed)%kod_b
            akod_c = ch_sed(jsed)%kod_c
            akod_d = ch_sed(jsed)%kod_d
          endif
    

            !!!! Deposition code moved up by KDW, commented out below, 01/21/22
  ...
  !!	Gravel deposition
        x = ch_sed(jsed)%pdep_alt * 1.055 * 1000. * ch_hyd(jhyd)%l * vgra / (vc * rchdep) ! added "pdep_alt *", KDW 10/25/21
          if (x > 20.) x = 20. 
        pdep = min((1. - exp(-x)), 1.)
          depgra = grain * pdep
  
  !!	sand deposition
        x = ch_sed(jsed)%pdep_alt * 1.055 * 1000. * ch_hyd(jhyd)%l * vsan / (vc * rchdep)! added "pdep_alt *", KDW 10/25/21
          if (x > 20.) x = 20.
        pdep = min((1. - exp(-x)), 1.)
          depsan = sanin * pdep
  
  !!	Silt deposition
        x = ch_sed(jsed)%pdep_alt * 1.055 * 1000. * ch_hyd(jhyd)%l * vsil / (vc * rchdep)! added "pdep_alt *", KDW 10/25/21
          if (x > 3.) x = 3. !Temporary, KDW, changed from > 20 and = 20 to > 3 and = 3, 02/03/22
        pdep = min((1. - exp(-x)), 1.)
  
        depsil = silin * pdep
  
  !!	Clay deposition
        x = ch_sed(jsed)%pdep_alt * 1.055 * 1000. * ch_hyd(jhyd)%l * vcla / (vc * rchdep)! added "pdep_alt *", KDW 10/25/21
          if (x > 2.) x = 2. !Temporary, KDW, changed from > 20 and = 20 to > 2 and = 2, 02/03/22
        pdep = min((1. - exp(-x)), 1.)
  
        depcla = clain * pdep
  
  !!	Small aggregates deposition
        x = ch_sed(jsed)%pdep_alt * 1.055 * 1000. * ch_hyd(jhyd)%l * vsag / (vc * rchdep)! added "pdep_alt *", KDW 10/25/21
          if (x > 4.) x = 4. !Temporary, KDW, changed from > 20 and = 20 to > 4 and = 4, 02/03/22
        pdep = min((1. - exp(-x)), 1.)
  
        depsag = sagin * pdep
  
  !!	Large aggregates deposition
        x = ch_sed(jsed)%pdep_alt * 1.055 * 1000. * ch_hyd(jhyd)%l * vlag / (vc * rchdep)! added "pdep_alt *", KDW 10/25/21
          if (x > 20.) x = 20.
        pdep = min((1. - exp(-x)), 1.)
  ...
  !!    Remaining is deposited in the channel
        ...
        sedin  = sedin - dep
        if (sedin < 1.e-6) sedin = 0.0 ! KDW, 01/30/22
      grain  = grain - depgra
      sanin  = sanin - depsan
      silin  = silin - depsil
      clain  = clain - depcla
  ...
  !!    Set apart the top 1mm of previously deposited sediment to erode first - KDW
    ...
    ! fines_wtclm = clain + silin + sagin + sanin + 0.0001 ! KDW moved up 2/17/22
...
!!    Set apart the top 1mm of previously deposited sediment to erode first - KDW  
  ...
  !     lagin  = lagin + deg1lag - deplag
      ...
      !!! Added because above code commented out, KDW 01/21/22
      sedin  = sedin + degrte + bnkrte + deg1
      grain  = grain + deggra + bnkgra + deg1gra
      sanin  = sanin + degsan + bnksan + deg1san
      silin  = silin + degsil + bnksil + deg1sil
      clain  = clain + degcla + bnkcla + deg1cla
      sagin  = sagin + deg1sag
      lagin  = lagin + deg1lag
      !!! End new code KDW
      ...
      outfract = rtwtr_d / qdin ! changed rtwtr to rtwtr_d, KDW 04/04/22
  ...
  !! compute changes in channel dimensions
        if (bsn_cc%deg == 1) then
          ...
          !! Copied down from lines in if qdin > 0.01 loop - KDW 01/31/22
              fines = ch(jrch)%depclach + ch(jrch)%depsilch + ch(jrch)%depsagch + ch(jrch)%depsanch
              ! aer_mass = .001 * 1 * (ch_vel(jrch)%wid_btm * 1000 * ch_hyd(jhyd)%l * ch_sed(jsed)%bed_bd) ! depth of aer layer is 1 ! moved outside qdin if statement, KDW 01/31/22
              ! anaer_mass = .1 * (ch_vel(jrch)%wid_btm * 1000 * ch_hyd(jhyd)%l * ch_sed(jsed)%bed_bd) ! moved outside qdin if statement, KDW 01/31/22
              if (fines >= aer_mass) then
              ...
              sfarea_fines = ch(jrch)%depclach + ch(jrch)%depsilch * .2 + & 
                      ch(jrch)%depsagch * (1./15) + ch(jrch)%depsanch * .01
              ! fines_wtclm = clain + silin + sagin + sanin + 0.0001 ! moved up outside of if/else, KDW 02/17/22
              ! sfarea_wtclm = clain + silin * .2 + sagin * (1/15) + sanin * .01 + 0.0001
          !! End code copied down from within if qdin > 0.01
  ...
  ! What are water volumes in layers (and in newly deposited sediment)
      ...
      fines_newdep = depcla + depsil + depsag + depsan + 0.0001 !moved up by KDW on 02/18/22, needs to be before vol_h20_newdep
  ...
  ! P deposition
      if (fines_wtclm > 1.e-6) then !change from > 0 to > 1.e-6 on 12/13/21, KDW, also below in lines 951 and 956
          percmass_dep = fines_newdep / fines_wtclm ! percentage (by mass) of aerobic layer that is eroded
      else
          percmass_dep = 1
      endif
      if (sfarea_wtclm > 1.e-6) then
      ...
      ! if (percmass_dep < 0.) percmass_dep = 0. ! KDW 02/17/22
      ! if (percsfar_dep < 0.) percsfar_dep = 0. ! KDW 02/17/22
      ! if (percvol_dep < 0.) percvol_dep = 0. ! KDW 02/17/22
      ! if (percmass_dep > 1.0) percmass_dep = 1.0 ! KDW 02/17/22
      ! if (percsfar_dep > 1.0) percsfar_dep = 1.0 ! KDW 02/17/22
      ! if (percvol_dep > 1.0) percvol_dep = 1.0 ! KDW 02/17/22

      orgp_dep = percsfar_dep * orgp_wtclm ! KDW, changed from "percmass *" to "percsfar *"  02/17/22
      ...
      if (fines_newdep >= aer_mass + anaer_mass) then
            ...
            if (fines_newdep >= aer_mass) then ! new aerobic layer is entirely new deposition
                ...
                if (fines_newdep + aer_mass >= aer_mass + anaer_mass) then
                        mass_from_aer = (anaer_mass + aer_mass) - fines_newdep
                        new_minpa_anaer = new_minpa_anaer + minpa_aer * (mass_from_aer / aer_mass)
                        new_minps_anaer = new_minps_anaer + minps_aer * (mass_from_aer / aer_mass)
                        new_orgp_anaer = new_orgp_anaer + orgp_aer * (mass_from_aer / aer_mass)
                        new_disp_anaer = new_disp_anaer + disp_aer * (mass_from_aer / aer_mass) 
                else
                        new_minpa_anaer = new_minpa_anaer + minpa_aer
                        new_minps_anaer = new_minps_anaer + minps_aer
                        new_orgp_anaer = new_orgp_anaer + orgp_aer
                        new_disp_anaer = new_disp_anaer + disp_aer

                        mass_still_anaer = anaer_mass - fines_newdep
                        new_minpa_anaer = new_minpa_anaer + minpa_anaer * (mass_still_anaer / anaer_mass)
                        new_minps_anaer = new_minps_anaer + minps_anaer * (mass_still_anaer / anaer_mass)
                        new_orgp_anaer = new_orgp_anaer + orgp_aer * (mass_still_anaer / anaer_mass)
                        new_disp_anaer = new_disp_anaer + disp_aer * (mass_still_anaer / anaer_mass)
                end if
            else
                  ...
                  mass_from_aer = aer_mass - mass_still_aer
                        
                  new_minpa_aer = new_minpa_aer + minpa_aer * (mass_still_aer / aer_mass)
                  new_minps_aer = new_minps_aer + minps_aer * (mass_still_aer / aer_mass)
                  new_orgp_aer = new_orgp_aer + orgp_aer * (mass_still_aer / aer_mass)
                  new_disp_aer = new_disp_aer + disp_aer * (mass_still_aer / aer_mass)

                  new_minpa_anaer = minpa_aer * (mass_from_aer / aer_mass)
                  new_minps_anaer = minps_aer * (mass_from_aer / aer_mass)
                  new_orgp_anaer = orgp_aer * (mass_from_aer / aer_mass)
                  new_disp_anaer = disp_aer * (mass_from_aer / aer_mass)
  
                  mass_still_anaer = anaer_mass - fines_newdep
                  new_minpa_anaer = new_minpa_anaer + minpa_anaer * (mass_still_anaer / anaer_mass)
                  new_minps_anaer = new_minps_anaer + minps_anaer * (mass_still_anaer / anaer_mass)
                  new_orgp_anaer = new_orgp_anaer + orgp_anaer * (mass_still_anaer / anaer_mass)
                  new_disp_anaer = new_disp_anaer + disp_anaer * (mass_still_anaer / anaer_mass)
            end if
      end if
  ...
  ! How much of each layer (aerobic and anaerobic) is left after erosion, how much is previously deposited "fines", and surface areas
      if (bed_ero <= aer_mass) then
            ...
            new_minpa_aer = new_minpa_aer * (1 - percsfar_ero_aer) + new_minpa_anaer * (bed_ero / anaer_mass)
            new_minps_aer = new_minps_aer * (1 - percsfar_ero_aer) + new_minps_anaer * (bed_ero / anaer_mass)
            new_orgp_aer = new_orgp_aer * (1 - percsfar_ero_aer) + new_orgp_anaer * (bed_ero / anaer_mass)
            new_disp_aer = new_disp_aer * (1 - percmass_ero_aer) + new_disp_anaer * (bed_ero / anaer_mass)

            new_minpa_anaer = new_minpa_anaer * (1 - (bed_ero / anaer_mass)) + bed_ero * ch(jrch)%ext_minpa / 1000
            new_minps_anaer = new_minps_anaer * (1 - (bed_ero / anaer_mass)) + bed_ero * ch(jrch)%ext_minps / 1000
            new_orgp_anaer = new_orgp_anaer * (1 - (bed_ero / anaer_mass)) + bed_ero * ch(jrch)%ext_orgp / 1000
            new_disp_anaer = new_disp_anaer * (1 - (bed_ero / anaer_mass)) + bed_ero * ch(jrch)%ext_disp / 1000
      else
            if (bed_ero <= anaer_mass) then
                ...
                rmn_anaer_mass = anaer_mass - (bed_ero - aer_mass)
                rmn_anaer_sfar = sfarea_anaer - (sfarea_ero - sfarea_aer)

                new_minpa_aer = new_minpa_anaer * (1 - percsfar_ero_anaer) * (aer_mass / rmn_anaer_mass)
                new_minps_aer = new_minps_anaer * (1 - percsfar_ero_anaer) * (aer_mass / rmn_anaer_mass)
                new_orgp_aer = new_orgp_anaer * (1 - percsfar_ero_anaer) * (aer_mass / rmn_anaer_mass)
                new_disp_aer = new_disp_anaer * (1 - percsfar_ero_anaer) * (aer_mass / rmn_anaer_mass)
    
                new_minpa_anaer = new_minpa_anaer * (1 - percsfar_ero_anaer) * (1-(aer_mass / rmn_anaer_mass)) &
                  + bed_ero * ch(jrch)%ext_minpa / 1000
                new_minps_anaer = new_minps_anaer * (1 - percsfar_ero_anaer) * (1-(aer_mass / rmn_anaer_mass)) &
                  + bed_ero * ch(jrch)%ext_minps / 1000
                new_orgp_anaer = new_orgp_anaer * (1 - percsfar_ero_anaer) * (1-(aer_mass / rmn_anaer_mass)) &
                  + bed_ero * ch(jrch)%ext_orgp / 1000
                new_disp_anaer = new_disp_anaer * (1 - percsfar_ero_anaer) * (1-(aer_mass / rmn_anaer_mass)) &
                  + bed_ero * ch(jrch)%ext_disp / 1000
            else
                if (bed_ero <= aer_mass + anaer_mass) then
                      ...
                      new_minpa_aer = new_minpa_anaer * (1 - percsfar_ero_anaer) + &
                        (aer_mass - rmn_anaer_mass) * ch(jrch)%ext_minpa / 1000
                      new_minps_aer = new_minps_anaer * (1 - percsfar_ero_anaer) + &
                        (aer_mass - rmn_anaer_mass) * ch(jrch)%ext_minps / 1000
                      new_orgp_aer = new_orgp_anaer * (1 - percsfar_ero_anaer) + &
                        (aer_mass - rmn_anaer_mass) * ch(jrch)%ext_orgp / 1000
                      new_disp_aer = new_disp_anaer * (1 - percsfar_ero_anaer) + &
                        (aer_mass - rmn_anaer_mass) * ch(jrch)%ext_disp / 1000
                      ...
                      new_disp_anaer = anaer_mass * ch(jrch)%ext_disp / 1000
                else
                      percmass_ero_aer = 1 ! percentage (by mass) of aerobic layer that is eroded
                      ...
                      new_minpa_aer = aer_mass * ch(jrch)%ext_minpa / 1000
                      new_minps_aer = aer_mass * ch(jrch)%ext_minps / 1000
                      new_orgp_aer = aer_mass * ch(jrch)%ext_orgp / 1000
                      new_disp_aer = aer_mass * ch(jrch)%ext_disp / 1000
      ...
      orgp_ero_aer = orgp_aer * percsfar_ero_aer! KDW, changed from "percmass *" to "percsfar *"  02/17/22
      disp_ero_aer = disp_aer * percmass_ero_aer
      minpa_ero_aer = percsfar_ero_aer * minpa_aer
      minps_ero_aer = percsfar_ero_aer * minps_aer
  
      orgp_ero_anaer = orgp_anaer * percsfar_ero_anaer! KDW, changed from "percmass *" to "percsfar *"  02/17/22
      ...
      ch(jrch)%aer_disp = 1000 * new_disp_aer / vol_h2o_aer ! KDW corrected from "/aer_mass", 03/21/22
      ...
      ch(jrch)%anaer_disp = 1000 * new_disp_anaer / vol_h2o_anaer ! KDW corrected from "/anaer_mass", 03/21/22
      ...
      ch_d(jrch)%phos_diff = ch_d(jrch)%phos_diff + disp_ero_aer + disp_ero_anaer - disp_dep ! KDW, 03/07/22
      if (jrch == 26)then !KDW debug 04/27/22
        x = ch_d(jrch)%phos_diff
      endif
      if (abs(ch_d(jrch)%phos_diff) > 10000)then!KDW debug 04/27/22
        x = ch_d(jrch)%phos_diff
      endif
  ...
  !!    Only bank erosion is assumed to contribute to channel erosion
     ...
     !!   ch_orgp(jrch) = bnkrte * ch_nut(jnut)%opco * 1000.
        ...
        return
      end subroutine ch_rtsed_kodatie2