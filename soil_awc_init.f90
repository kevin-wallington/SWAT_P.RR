...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      depth_prev = 0.
      sumpor = 0.
      soil(isol)%sumul = 0. ! KDW, 11/19/21
      soil(isol)%sumfc = 0. ! KDW, 11/19/21
      soil(isol)%sumwp = 0. ! KDW, 11/19/21
      do ly = 1, nly 
      ...
      if (soil(isol)%ffc > 1.) then
        !soil(isol)%wat_tbl = (soil(isol)%det_lag - soil(isol)%ffc *   & ! commented out and replaced with below by KDW 11/19/21
        !  soil(isol)%sumfc) / soil(isol)%phys(nly)%d
        soil(isol)%wat_tbl = soil(isol)%phys(nly)%d - (soil(isol)%ffc *   &
          soil(isol)%sumfc) / soil(isol)%phys(nly)%por
      else
        soil(isol)%wat_tbl = 0.
      end if
