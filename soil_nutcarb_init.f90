...
!!    this subroutine initializes soil chemical properties
    ...
    !         solp = sol1(isol)%mp(j)%lab / sol(isol)%phys(j)%conv_wt !* 1000000. !-commented out *1000000 by KDW
    ...
    !     !! Set Stable pool based on dynamic coefficient
	  !   if (bsn_cc%sol_P_model == 1) then  !! From White et al 2009 - change from ==0 to ==1 - KDW
    !         !! convert to concentration for ssp calculation
	  !       actp = sol1(isol)%mp(j)%act / sol(isol)%phys(j)%conv_wt !* 1000000. !-commented out *1000000 by KDW
		!     solp = sol1(isol)%mp(j)%lab / sol(isol)%phys(j)%conv_wt !* 1000000. !-commented out *1000000 by KDW
    ...
    !   end do
	  ...
	  do j = 1, nly
        ...
        ! C:N ratios changed back to 14:1 by KDW following Cibin, C:P (112:1) changed so N:P = 8, KDW 02/17/22
        ...
        sol1(isol)%tot(j)%n = sol1(isol)%tot(j)%c / 14.     !assume 10:1 C:N ratio
        sol1(isol)%tot(j)%p = sol1(isol)%tot(j)%c / 112.    !assume 100:1 C:P ratio
        ...
        sol1(isol)%hp(j)%n = sol1(isol)%hp(j)%c / 14.                       !assume 10:1 C:N ratio, 
        !sol1(isol)%hp(j)%p = sol1(isol)%hp(j)%c / 80.                       !assume 80:1 C:P ratio ! moved below and combined with hs pool since no hs pool for P, KDW 10/15/21
        ...
        sol1(isol)%hs(j)%n = sol1(isol)%hs(j)%c / 14.                   !assume 10:1 C:N ratio
        !sol1(isol)%hs(j)%p = sol1(isol)%hs(j)%c / 80.                   !assume 80:1 C:P ratio !commented out by KDW b/c there is no such pool used in model, 10/15/21

        sol1(isol)%hp(j)%p = (sol1(isol)%hp(j)%c + sol1(isol)%hs(j)%c) / 112.                       !assume 80:1 C:P ratio ! KDW 10/15/21
