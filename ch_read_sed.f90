...
!!    apply to all reservoirs in the watershed. 
    ...
    do ich = 1, db_mx%ch_sed
      ...
      !if (eof < 0) exit ! moved to end of do loop, so below checks are conducted first, KDW 10/30/21
...
!!  Base on Hanson and Simon, 2001
      ...
      if (eof < 0) exit ! moved from beginning of do loop, so below checks are conducted first, KDW 10/30/21 
    end do
    exit
  end do
  end if
