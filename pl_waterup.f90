...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: yy             !          | 
      real :: rel_ep         ! relative transpiration (to max) due to water stress
      ...
      if (epmax(ipl) <= 1.e-6) then
        ...
        do k = 1, soil(j)%nly
          ...
          if (soil(j)%phys(k)%st <= 0.4 * soil(j)%phys(k)%fc) then ! KDW 11/12/21 added for water stress/drought sensitivity; see Sinclair et al. (Agroclimatology, 1998) and Verhoef and Egea (AFM, 2014)
            rel_ep = soil(j)%phys(k)%st / (0.4 * soil(j)%phys(k)%fc)
            wuse = rel_ep * wuse
          endif
          wuse = amin1 (wuse, soil(j)%phys(k)%st)
		  if (soil(j)%phys(k)%st < wuse) then ! KDW moved up from below, commented out below, 11/12/21
