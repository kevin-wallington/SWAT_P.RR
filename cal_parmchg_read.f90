      ...
      do i = 1, mcal
        ...
        if (nspu > 0) then
          ...
          if (cal_upd(i)%num_tot == 0) then
            ...
            case ("rte") ! changed from "cha", KDW 06/25
              cal_upd(i)%num_elem = db_mx%ch_hyd ! was "= sp_ob%chan", KDW, 06/25
            case ("sed") ! changed from "cha", KDW 06/25
              !cal_upd(i)%num_elem = db_mx%ch_sed ! was "= sp_ob%chan", KDW, 06/25
              !!! Temporarily replaces above, KDW 01/18/21
              cal_upd(i)%num_elem = 1
            case ("swq")
              cal_upd(i)%num_elem = db_mx%ch_nut ! was "= sp_ob%chan", KDW, 06/25
            case ("res")
