...
!!    chl_a(:)    |mg/L          |chlorophyll-a concentration in water yield
!!                               |on current day in HRU - unit changed from micrgram to mg, KDW 07/15
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use basin_module
      use hru_module, only : hru, ihru, tmpav, qdr, sedorgn, surqno3, cbodu, doxq, chl_a, sedyld, enratio, qday, &
        latno3, latorgn, tileno3, tileorgn, latsed, tilesed
      use soil_module
      ...
      j = ihru
        ...
        if (qdr(j) > 1.e-4) then
            tp = 100. * (sedorgn(j) + surqno3(j) + latno3(j) + latorgn(j) &
              + tileno3(j) + tileorgn(j)) / qdr(j)    !100*kg/ha/mm = ppm (=mg/L, KDW), added subsurface N, 12/13/21- KDW
          !chl_a(j) = .1 * tp                                ! assume chlorophyll a is 0.01 total p
          chl_a(j) = .04 * tp                                ! changed to match SWAT 2012 since the above is unclear (0.1 or 0.01?), in mg/L rather than micrograms/L - KDW, 07/15
 
          !! calculate organic carbon loading to main channel
          org_c = 0.
          !org_c = (soil1(j)%tot(1)%c / 100.) * enratio*sedyld(j) * 1000. !replaced with below by KDW, should be percent carbon, not carbon in kg/ha, KDW 12/13/21
          if (hru(j)%hyd%erorgn > .001) then !enrichment ratio calc added by KDW
            er = hru(j)%hyd%erorgn
          else
            er = enratio
          end if
          org_c = (soil1(j)%cbn(1) / 100.) * er*(sedyld(j) + latsed(j) + tilesed(j)) * 1000. ! latsed and tile sed added by KDW 12/13/21
